        ...
        while index < update_data_length {
                ...
                && data == accumulator_magic_bytes(ACCUMULATOR_MAGIC)
            {
                ...
                while index_2 < number_of_updates {
                    ...
                    let price_feed_id_index = find_index_of_price_feed_id(
                        price_feed_ids,
                        price_feed_id
                    );
                    ...
                    if price_feed_id_index == price_feed_ids.len || price_feeds.get(price_feed_id_index).unwrap().id != ZERO_B256 {
                        continue;
                    }
                    ...
                    if price_feed.publish_time >= min_publish_time && price_feed.publish_time <= max_publish_time {
                        price_feeds.push(
                            price_feed
                        )
                    }
